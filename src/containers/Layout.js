...
class Layout extends Component {
    ...
    onSubmitHandler = e => {
        e.preventDefault();
        fetch(`https://us-central1-github-568d6.cloudfunctions.net/expressApi/api/users/${this.state.name}`)
            .then(response => {
                if (response.ok) {
            ...
            .then(profile => {
                if (profile.message !== "Not Found") {
                    fetch(`https://us-central1-github-568d6.cloudfunctions.net/expressApi/api/repos/${this.state.name}`)
                        .then(data => data.json())
                        .then(data => {
